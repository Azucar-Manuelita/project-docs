## Apartado 1

### Clavijo Tocasuchyl Sebastian

**Rol a desempeñar:**  
Me gustaría desempeñar el rol de quien profundiza en la arquitectura y diseño del software, participando en la toma de decisiones técnicas clave y asegurando que las soluciones sean estructuradas, escalables y mantenibles. También me interesa aportar en la organización general del proyecto para mantener claridad en los objetivos y tareas.

**Fortalezas personales:**  
Considero que tengo buena capacidad analítica y facilidad para comprender conceptos técnicos, lo que me permite adaptarme rápidamente a nuevos temas. Además, soy responsable con mis compromisos y me gusta entender a fondo los problemas antes de proponer soluciones.

**Aspectos a fortalecer:**  
Debo mejorar mi gestión del tiempo para evitar acumulación de tareas y trabajar con mayor anticipación. También quiero fortalecer mi comunicación dentro del equipo, especialmente al momento de expresar ideas técnicas de manera clara y estructurada.

### Otalora De La Cuadra Samuel

**Rol a desempeñar:**  
Quiero desempeñar un rol orientado a el analissi y desarrollo de software, participando en definicion de requerimientos, desarrollo del software, estructuracion logica e implementacion tecnica, asegurando que el proyecto sea coherente y bien estructurado

**Fortalezas personales:**  
Buena capacidad de solucion de problemas y capacidad de adaptabilidad cuando ocurre un cambio inesperado

**Aspectos a fortalecer:**  
Quiero mejorar la planificación y estimación de tareas para organizar mejor el tiempo dentro del proyecto. También busco fortalecer mis habilidades de trabajo colaborativo, en el seguimiento de metodologías de desarrollo de manera más disciplinada.

### Caucali Junco Nicolas

**Rol a desempeñar :**  Me gustaría ser de aquellos quienes profundizan en la arquitectura y diseño del programa, desarrollar las soluciones a las neceidades que se presenten durante el proyecto.

**Fortalezas :**  Mi creatividad e ingenio pueden ser utiles a la hora de tomar decisiones.

**Aspectos a mejorar :**  Siento que tengo que aprender a cooperar y seguír una metodología concreta ya que suelo ser desordenado y a veces tiendo a procrastinar.

### Fuentes Ramos Nicolas

**Rol a desempeñar:**
Me gustaria desempeñar el rol del encargado de la parte de profundizacion y diseño del codigo para mantenerlo lo mas limpio y logico posible para su entendimiento posterior o en el enfoque de testeos y pruebas de calidad del mismo.

**Fortalezas personales:**
Considero que se me facilita el entendimiento de logica basica del codigo e incluyo una gran curiosdad por entender como va relacionada cada cosa con el resto para matener un arquitectura entendible, ademas de ser curioso acerca de como funciona cada parte del mismo para asegurarme que no queden partes de codigo extrañas o mal hechas.

**Aspectos a fortalecer:**
Debo mejorar el conocimiento acerca de los distintos lenguajes para en caso necesario poder solucionar problemas sin la necesidad de herramientas externas o largas busquedas en internet para solventar el problema que veo en el codugo, a su misma vez debo mejorar mi forma de hacer codigo de modo mas ordenado y entendible.


## Apartado 2

### Expectativas

- Que el equipo mantenga una comunicación clara, respetuosa y constante durante todo el proyecto.\
- Que todos los integrantes participen activamente y cumplan con las tareas asignadas.\
- Poder llevara acabo la metodología Kanban de la mejor manera durante el transcurso del proyecto.\
- Que el repositorio de GitHub se mantenga ordenado, con commits claros y descriptivos.\
- Que el producto final cumpla con los requisitos establecidos y tenga buena calidad tanto funcional como técnica.\
- Que exista apoyo mutuo en caso de dificultades técnicas o limitaciones de tiempo.\

### Compromisos

- Se decidió usar la metodología ágil Kanban, ya que nos brinda mayor flexibilidad en la gestión de tareas y facilita la adaptación a cambios durante el desarrollo del proyecto.\
- Se realizará una reunión formal una vez por semana, preferiblemente los sábados, y se complementará con reuniones cortas los días de clase si es necesario.\
- Reglas para el manejo de GitHub:\
  - No trabajar directamente sobre la rama main.\
  - Crear ramas independientes para cada funcionalidad o corrección.\
  - Informar al equipo antes de realizar un push importante.\
  - Realizar pull requests para revisión antes de fusionar cambios.\
  - Escribir mensajes de commit claros y descriptivos.\
- Si alguna tarea no puede completarse en la fecha estipulada, se deberá informar lo antes posible para recibir apoyo o reorganizar la planificación.\
- En caso de no poder asistir a una reunión, el integrante deberá avisar con anticipación para buscar una solución alternativa.\
- Las decisiones importantes del equipo se tomarán de manera democrática, permitiendo que todos los integrantes participen y voten.\
- Cada integrante se compromete a cumplir con los tiempos establecidos y asumir responsabilidad sobre sus tareas asignadas.
